...
router.post('/', function (req, res, next) {
    ...
    * fields: 테이블의 필드 정보
    */
    mysqlDB.query('select * from account where id=? and pw=SHA1(?)', [userId, userPw], function (err, rows, fields) {
        if (!err) {                         //오류가 없을 때 아래의 코드 실행
            if (rows[0] != undefined) {     //row가 존재하면 회원정보 삭제

                mysqlDB.query('delete from account where id=?', [userId], function (err, rows, fields) {
                    if (!err) {             //오류가 없을 때 아래의 코드 실행
